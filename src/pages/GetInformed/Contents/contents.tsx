import BeneficiosDePararDeFumarImg from "@assets/GetInformed/Contents/heros/beneficios-de-parar-de-fumar-conteudo.jpg";
import CigarroEletronicoMitosEVerdadesImg from "@assets/GetInformed/Contents/heros/cigarro-eletronico-mitos-e-verdades-conteudo-contents.svg";
import ComparacaoEntreMetodosDeCessacaoDoTabagismoImg from "@assets/GetInformed/Contents/heros/comparacao-entre-metodos-de-cessação-do-tabagismo-conteudo-contents.svg";
import EntendendoOTabagismoImg from "@assets/GetInformed/Contents/heros/entendendo-o-tabagismo-conteudo.jpg";
import ImpactoDoCigarroImg from "@assets/GetInformed/Contents/heros/impacto-do-cigarro-conteudo.png";
import OPapelDasMidiasSociaisNaPromocaoDoTabagismoEntreOsJovensImg from "@assets/GetInformed/Contents/heros/o-papel-das-midias-sociais-na-promocao-do-tabagismo-entre-os-jovens-conteudo-contents.svg";
import GabrielaEscritoraImg from "@assets/GetInformed/Contents/writers/gabriela-escritora.jpg";
import RobertaEscritoraImg from "@assets/GetInformed/Contents/writers/roberta-escritora.jpg";

const contents = {
  "entendendo-o-tabagismo": {
    title: "Entendendo o Tabagismo",
    subtitle: "Desvende os seus perigos e abrace a esperança",
    heroImage: EntendendoOTabagismoImg,
    writer: {
      photo: RobertaEscritoraImg,
      name: "Roberta Braz",
      articleDate: "20 de julho de 2024",
    },
    article: `
            <section>
              <p>
                O tabagismo é o ato de se consumir cigarros ou outros produtos
                que contenham tabaco, cuja droga ou princípio ativo é a
                nicotina. A 
                <a href="https://www.who.int/" target="_blank">
                  Organização Mundial da Saúde (OMS)
                </a> 
                considera o tabagismo uma pandemia silenciosa, causando danos
                irreversíveis à saúde dos fumantes e das pessoas ao seu redor.
              </p>
              <p>
                De acordo com a <a href="https://portal.fiocruz.br/" target="_blank">Fundação Oswaldo Cruz (Fiocruz)</a>, a fumaça do
                cigarro é uma mistura de cerca de 4.720 substâncias tóxicas,
                divididas em duas fases fundamentais: a fase gasosa e a fase
                particulada. A fase gasosa inclui monóxido de carbono, amônia,
                cetonas, formaldeído, acetaldeído e acroleína. A fase
                particulada contém nicotina e alcatrão. Essas substâncias
                tóxicas afetam diversos sistemas e órgãos do corpo, incluindo
                mais de 60 substâncias cancerígenas, destacando-se as seguintes:
              </p>
              <ul>
                <li>Nicotina: causa dependência e é cancerígena.</li>
                <li>
                  Benzopireno: facilita a combustão no papel que envolve o fumo.
                </li>
                <li>
                  Substâncias radioativas: incluem polônio-210 e carbono-14.
                </li>
                <li>Agrotóxicos: como o Dicloro-Difenil-Tricloroetano(DDT).</li>
                <li>Solvente: benzeno.</li>
                <li>
                  Metais pesados: chumbo e cádmio (um cigarro contém de 1 a 2 mg
                  de cádmio, que se acumula no fígado, rins e pulmões, com
                  meia-vida de 10 a 30 anos, levando à perda da capacidade
                  ventilatória dos pulmões, além de causar dispneia, enfisema,
                  fibrose pulmonar, hipertensão, e câncer nos pulmões, próstata,
                  rins e estômago).
                </li>
                <li>
                  Níquel e arsênico: armazenam-se no fígado, rins, coração,
                  pulmões, ossos e dentes, podendo resultar em gangrena dos pés
                  e danos ao miocárdio.
                </li>
              </ul>
              <p>
                O tabaco pode ser consumido de diversas maneiras, dependendo da
                sua forma de apresentação: inalado (cigarro, charuto, cigarro de
                palha), aspirado (rapé) e mascado (fumo-de-rolo). No entanto,
                independentemente da forma de uso, ele é prejudicial à saúde.
              </p>
              <p>
                Além disso, o tabagismo está associado a cerca de 50 doenças
                diferentes, com destaque para as cardiovasculares como
                hipertensão, infarto, angina e derrame. É também responsável por
                muitas mortes devido a cânceres como os de pulmão, boca,
                laringe, esôfago, estômago, pâncreas, rim e bexiga, além de
                causar doenças respiratórias obstrutivas como bronquite crônica
                e enfisema pulmonar. O tabaco compromete as defesas naturais do
                organismo, aumentando a suscetibilidade a doenças como gripe e
                tuberculose, e é associado à impotência sexual.
              </p>
            </section>
            <section>
              <h3>Por que as pessoas começam a fumar?</h3>
              <p>
                Existem vários fatores que levam as pessoas a experimentar
                cigarros ou outros derivados do tabaco:
              </p>
              <ul>
                <li>
                  A publicidade desempenha um papel significativo, sendo
                  amplamente veiculada nos meios de comunicação. Para os jovens,
                  esse impacto é ainda mais pronunciado, pois são influenciados
                  não apenas pela publicidade, mas também por pais, professores,
                  ídolos e amigos durante a fase crucial de construção da sua
                  personalidade, antes dos 19 anos.
                </li>
                <li>
                  Pesquisas indicam que a maioria dos adolescentes que fumam
                  iniciou o hábito nessa faixa etária, muitas vezes buscando
                  autoafirmação. O tabaco não se enquadra no conceito moderno de
                  moda, que abrange música, roupas, gostos, gírias e danças.
                </li>
                <li>
                  No passado, a publicidade manipulava psicologicamente
                  diferentes grupos sociais, como adolescentes, mulheres e
                  pessoas de baixa renda, sugerindo que o tabagismo era mais
                  comum e socialmente aceito do que era na realidade. Comerciais
                  frequentemente retratavam pessoas atraentes, jovens e
                  bem-sucedidas desfrutando de momentos de lazer ou convívio
                  social, associados ao cigarro. No Brasil, essa forma de
                  publicidade foi proibida pela Lei 10.167, que restringe a
                  propaganda de cigarros e seus derivados.
                </li>
              </ul>
            </section>
            <section>
              <h3>Fumar durante a gravidez: riscos e consequências</h3>
              <p>
                Fumar durante a gravidez apresenta sérios riscos tanto para a
                gestante quanto para o bebê, aumentando significativamente a
                mortalidade fetal e infantil. Entre os riscos associados estão:
              </p>
              <ul>
                <li>Abortos espontâneos.</li>
                <li>Nascimentos prematuros.</li>
                <li>Bebês com baixo peso ao nascer.</li>
                <li>Mortes fetais e de recém-nascidos.</li>
                <li>Gravidez ectópica.</li>
                <li>Descolamento prematuro da placenta.</li>
                <li>Placenta prévia.</li>
                <li>Episódios de sangramento.</li>
              </ul>
              <p>
                Comparativamente, gestantes que fumam enfrentam mais
                complicações durante o parto e têm o dobro de chances de ter
                bebês com peso e comprimento abaixo do esperado em comparação
                com gestantes não fumantes.
              </p>
            </section>
            <section>
              <h3>O que é ser um fumante passivo?</h3>
              <p>
                Um fumante passivo é alguém que convive com fumantes e inala a
                fumaça dos derivados do tabaco em ambientes fechados.
              </p>
              <p>
                A Poluição Tabagística Ambiental (PTA) refere-se à fumaça dos
                produtos de tabaco em locais fechados, identificada pela
                Organização Mundial de Saúde (OMS) como a principal fonte de
                poluição nesses ambientes. Pesquisas indicam que o tabagismo
                passivo é considerado a terceira maior causa de morte evitável
                no mundo, ficando atrás apenas do tabagismo ativo e do consumo
                excessivo de álcool.
              </p>
              <p>
                Indivíduos não fumantes expostos à fumaça do tabaco enfrentam um
                risco elevado de desenvolver doenças relacionadas ao tabagismo.
                Quanto mais tempo permanecem expostos à poluição tabagística
                ambiental, maior é a probabilidade de adoecer. As crianças são
                particularmente vulneráveis devido à sua frequência respiratória
                elevada, enfrentando impactos significativos na saúde, como
                bronquite, pneumonia, asma e infecções do ouvido médio.
              </p>
              <p>
                Lembre-se dos benefícios: Parar de fumar não só melhora sua
                saúde imediata e a longo prazo, reduzindo o risco de diversas
                doenças graves, mas também aumenta sua qualidade de vida e te
                aproxima de uma vida mais plena.
              </p>
              <p>
                Você não está sozinho nessa jornada! Junte-se a nós, no
                movimento Cigarro Zero, para construir um futuro livre do
                tabagismo, onde a saúde e o bem-estar sejam prioridades para
                todos.
              </p>
            </section>
    `,
  },
  "impacto-do-cigarro": {
    title: "Impacto do Cigarro na Saúde: Doenças e Complicações",
    subtitle:
      "O tabagismo causa diversas doenças graves, incluindo problemas respiratórios, cardiovasculares e diferentes tipos de câncer",
    heroImage: ImpactoDoCigarroImg,
    writer: {
      photo: GabrielaEscritoraImg,
      name: "Gabriela Buarque",
      articleDate: "22 de julho de 2024",
    },
    article: `
            <section>
              <p className="paragraph">
                O tabagismo é responsável por uma ampla gama de doenças graves,
                afetando praticamente todos os sistemas do corpo humano. Abaixo,
                detalhamos algumas das condições mais comuns associadas ao uso
                do cigarro.
              </p>
            </section>
            <section>
              <h3>Doenças Respiratórias</h3>
              <p className="paragraph">
                O sistema respiratório é um dos mais afetados pelo tabagismo. Os
                principais problemas incluem:
              </p>
              <ul>
                <li>
                  <strong>Bronquite Crônica:</strong> Caracterizada pela
                  inflamação persistente dos brônquios, a bronquite crônica leva
                  à produção excessiva de muco, tosse contínua e dificuldade
                  para respirar.
                </li>
                <li>
                  <strong>Enfisema Pulmonar:</strong> Esta condição envolve a
                  destruição dos alvéolos, as pequenas bolsas de ar nos pulmões,
                  resultando em uma capacidade reduzida de oxigenação do sangue
                  e falta de ar severa.
                </li>
                <li>
                  <strong>Asma:</strong> Embora a asma possa ser uma condição
                  preexistente, o tabagismo pode agravar significativamente os
                  sintomas e aumentar a frequência e a gravidade das crises
                  asmáticas.
                </li>
                <li>
                  <strong>Câncer de Pulmão:</strong> O mais conhecido e mortal
                  dos cânceres relacionados ao tabagismo. Fumar é a principal
                  causa de câncer de pulmão, que tem altas taxas de mortalidade
                  devido à sua agressividade e diagnóstico frequentemente
                  tardio.
                </li>
              </ul>
            </section>
            <section>
              <h3>Doenças Cardiovasculares</h3>
              <p>
                O impacto do tabagismo no sistema cardiovascular é extenso e
                pode levar a várias condições graves, tais como:
              </p>
              <ul>
                <li>
                  <strong>Infarto do Miocárdio:</strong> Conhecido como ataque
                  cardíaco, ocorre quando o fluxo sanguíneo para uma parte do
                  coração é bloqueado, danificando o músculo cardíaco.
                </li>
                <li>
                  <strong>Angina:</strong> Dor no peito resultante da falta de
                  oxigênio no coração, frequentemente um precursor de um ataque
                  cardíaco.
                </li>
                <li>
                  <strong>Acidente Vascular Cerebral (AVC):</strong> O tabagismo
                  contribui para a formação de coágulos sanguíneos e o
                  endurecimento das artérias, aumentando significativamente o
                  risco de derrames.
                </li>
                <li>
                  <strong>Hipertensão:</strong> A pressão alta é uma condição
                  comum em fumantes, aumentando o risco de várias doenças
                  cardiovasculares.
                </li>
                <li>
                  <strong>Arteriosclerose:</strong> Este endurecimento e
                  estreitamento das artérias pode levar a graves problemas
                  circulatórios e é exacerbado pelo tabagismo.
                </li>
              </ul>
            </section>
            <section>
              <h3>Cânceres</h3>
              <p>
                Além do câncer de pulmão, o tabagismo aumenta o risco de vários
                outros tipos de câncer:
              </p>
              <ul>
                <li>
                  <strong>Câncer de Boca:</strong> O tabaco danifica as células
                  da boca, aumentando o risco de lesões malignas.
                </li>
                <li>
                  <strong>Câncer de Laringe e Faringe:</strong> O contato direto
                  com a fumaça pode causar cânceres nessas áreas do trato
                  respiratório.
                </li>
                <li>
                  <strong>Câncer de Esôfago:</strong> O tabagismo é um fator de
                  risco significativo para este tipo de câncer.
                </li>
                <li>
                  <strong>Câncer de Estômago:</strong> Substâncias cancerígenas
                  do tabaco podem causar danos ao revestimento do estômago.
                </li>
                <li>
                  <strong>Câncer de Pâncreas:</strong> Altamente letal, este
                  câncer tem uma forte associação com o tabagismo.
                </li>
                <li>
                  <strong>Câncer de Fígado:</strong> O tabaco contribui para o
                  desenvolvimento de câncer hepático.
                </li>
                <li>
                  <strong>Câncer de Bexiga e Rim:</strong> Toxinas do cigarro
                  são filtradas pelos rins e armazenadas na bexiga, aumentando o
                  risco de câncer nesses órgãos.
                </li>
                <li>
                  <strong>Câncer de Colo do Útero:</strong> Mulheres fumantes
                  têm um risco maior de desenvolver câncer cervical.
                </li>
              </ul>
            </section>
            <section>
              <h3>Outras Doenças</h3>
              <p>
                O tabagismo também é um fator significativo em várias outras
                condições de saúde:
              </p>
              <ul>
                <li>
                  <strong>Osteoporose:</strong> Fumantes têm maior risco de
                  perda óssea e fraturas.
                </li>
                <li>
                  <strong>Catarata:</strong> O fumo pode causar a opacificação
                  do cristalino, levando à perda de visão.
                </li>
                <li>
                  <strong>Úlceras Gástricas:</strong> A irritação do
                  revestimento do estômago pelo tabaco pode resultar em úlceras.
                </li>
                <li>
                  <strong>Impotência Sexual e Infertilidade:</strong> O
                  tabagismo pode prejudicar a função sexual e a capacidade
                  reprodutiva em ambos os sexos.
                </li>
                <li>
                  <strong>Doenças Periodontais:</strong> Fumar contribui para
                  doenças das gengivas e perda dentária.
                </li>
                <li>
                  <strong>Complicações na Gravidez:</strong> Incluem parto
                  prematuro, baixo peso ao nascer e síndrome da morte súbita
                  infantil.
                </li>
              </ul>
            </section>
            <section>
              <h3>Impacto do Tabagismo Passivo</h3>
              <p>
                O fumo passivo também traz sérios riscos, afetando não apenas os
                fumantes, mas também aqueles ao seu redor:
              </p>
              <ul>
                <li>
                  <strong>Crianças:</strong> A exposição ao fumo passivo aumenta
                  o risco de infecções respiratórias, pneumonia, bronquite, asma
                  e morte súbita infantil.
                </li>
                <li>
                  <strong>Adultos:</strong> Fumantes passivos estão em maior
                  risco de doenças cardíacas, AVC, câncer de pulmão e outras
                  doenças respiratórias.
                </li>
              </ul>
              <p>
                Dada a extensa gama de doenças associadas ao tabagismo, parar de
                fumar é uma das melhores decisões que você pode tomar para
                proteger sua saúde e a saúde daqueles ao seu redor. Existem
                diversos métodos e ferramentas disponíveis para ajudar nesse
                processo, e nunca é tarde para começar!
              </p>
            </section>
    `,
  },
  "beneficios-de-parar-de-fumar": {
    title: "Benefícios de Parar de Fumar",
    subtitle: "Uma nova vida te espera!",
    heroImage: BeneficiosDePararDeFumarImg,
    writer: {
      photo: RobertaEscritoraImg,
      name: "Roberta Braz",
      articleDate: "20 de julho de 2024",
    },
    article: `
            <section>
              <p>
                Parar de fumar é uma das decisões mais importantes que você pode
                tomar para melhorar sua saúde e qualidade de vida. Os benefícios
                de deixar o cigarro são vastos e começam a se manifestar quase
                imediatamente. Aqui estão alguns dos principais benefícios de
                parar de fumar:
              </p>
              <ul>
                <li>
                  Melhoria da saúde respiratória: dentro de 20 minutos após o
                  último cigarro, sua frequência cardíaca e pressão arterial
                  começam a cair. Em poucos dias, os níveis de monóxido de
                  carbono no sangue retornam ao normal, melhorando a capacidade
                  de transporte de oxigênio. Em semanas, a função pulmonar
                  melhora, e sintomas como tosse e falta de ar diminuem.
                </li>
                <li>
                  Redução do risco de doenças graves: parar de fumar reduz
                  significativamente o risco de desenvolver doenças graves como
                  câncer, doenças cardíacas, Acidente Vascular Cerebral (AVC),
                  doenças pulmonares crônicas e muitas outras condições
                  relacionadas ao tabagismo. A cada ano sem fumar, o risco de
                  ataque cardíaco diminui.
                </li>
                <li>
                  Melhoria na qualidade de vida: ex-fumantes frequentemente
                  relatam um aumento no nível de energia, melhor paladar e
                  olfato, e uma sensação geral de bem-estar. A respiração se
                  torna mais fácil, e a capacidade de realizar atividades
                  físicas melhora.
                </li>
                <li>
                  Saúde da pele: parar de fumar melhora a circulação sanguínea,
                  o que pode resultar em uma pele mais saudável e jovem. Reduzir
                  a exposição a toxinas do cigarro pode retardar o
                  envelhecimento precoce e melhorar a aparência da pele.
                </li>
                <li>
                  Saúde bucal: deixar de fumar melhora significativamente a
                  saúde bucal, reduzindo o risco de doenças gengivais, perda
                  dentária e câncer bucal. O hálito também se torna mais fresco
                  e os dentes menos amarelados.
                </li>
                <li>
                  Benefícios econômicos: ao longo do tempo o custo do cigarro
                  pode ser significativo. Parar de fumar libera recursos
                  financeiros que podem ser utilizados para outras necessidades
                  e desejos, contribuindo para uma melhor qualidade de vida.
                </li>
                <li>
                  Proteção para a família e amigos: você protege aqueles ao seu
                  redor da exposição ao fumo passivo, que pode causar uma série
                  de problemas de saúde, especialmente em crianças e adultos
                  mais vulneráveis.
                </li>
                <li>
                  Aumento da longevidade: aumenta a expectativa de vida. As
                  pessoas que deixam de fumar antes dos 40 anos reduzem o risco
                  de morte prematura relacionada ao tabagismo em cerca de 90%.
                </li>
                <li>
                  Benefícios psicológicos: melhora a saúde mental, reduzindo os
                  níveis de estresse e ansiedade. O orgulho e a autoestima
                  aumentam à medida que se alcançam marcos importantes no
                  processo de parar de fumar.
                </li>
              </ul>
            </section>
            <section>
              <h3>Benefícios imediatos</h3>
              <ul>
                <li>
                  20 minutos: a pressão arterial e a frequência cardíaca voltam
                  ao normal.
                </li>
                <li>
                  12 horas: o nível de monóxido de carbono no sangue se
                  normaliza.
                </li>
                <li>
                  2-12 semanas: a circulação melhora e a função pulmonar
                  aumenta.
                </li>
                <li>
                  3-9 meses: a tosse e a falta de ar diminuem
                  significativamente.
                </li>
              </ul>
            </section>
            <section>
              <h3>Benefícios a Longo Prazo</h3>
              <ul>
                <li>
                  5 anos: o risco de Acidente Vascular Cerebral (AVC) é reduzido
                  ao nível de um não fumante.
                </li>
                <li>
                  10 anos: a taxa de mortalidade por câncer de pulmão é reduzida
                  pela metade.
                </li>
                <li>
                  15 anos: o risco de doença cardíaca é igual ao de um não
                  fumante.
                </li>
              </ul>
            </section>
            <section>
              <h3>Como parar de fumar?</h3>
              <p>Existem diversas formas de parar de fumar, como:</p>
              <ul>
                <li>
                  Terapia de reposição de nicotina: utiliza produtos como
                  adesivos, pastilhas ou inaladores para reduzir os sintomas da
                  abstinência.
                </li>
                <li>
                  Medicamentos prescritos: auxiliam na redução da vontade de
                  fumar.
                </li>
                <li>
                  Terapia comportamental: ensina técnicas para lidar com o vício
                  e as situações que levam ao cigarro.
                </li>
                <li>
                  Grupos de apoio: oferecem um ambiente seguro para compartilhar
                  experiências e receber apoio de outras pessoas que estão
                  passando pela mesma situação.
                </li>
              </ul>
              <p>
                Lembre-se dos benefícios: parar de fumar não só melhora sua
                saúde imediata e a longo prazo, reduzindo o risco de diversas
                doenças graves, mas também aumenta sua qualidade de vida e te
                aproxima de uma vida mais plena.
              </p>
              <p>
                Você não está sozinho nessa jornada! Junte-se a nós, no
                movimento Cigarro Zero, para construir um futuro livre do
                tabagismo, onde a saúde e o bem-estar sejam prioridades para
                todos.
              </p>
            </section>
    `,
  },
  "comparacao-entre-metodos-de-cessação-do-tabagismo": {
    title: "Comparação Entre Métodos de Cessação do Tabagismo",
    subtitle: "Parar de fumar é possível e cada vez mais acessível",
    heroImage: ComparacaoEntreMetodosDeCessacaoDoTabagismoImg,
    writer: {
      photo: RobertaEscritoraImg,
      name: "Roberta Braz",
      articleDate: "29 de julho de 2024",
    },
    article: `
            <section>
              <p>
                A cada ano, milhões de pessoas no mundo tentam e falham em parar
                de fumar. Mas a boa notícia é que a cessação do tabagismo é
                possível e cada vez mais acessível. Parar de fumar não é apenas
                uma questão de saúde, mas também de qualidade de vida. Ao
                abandonar o cigarro, é possível reduzir significativamente o
                risco de doenças como câncer, doenças cardíacas e respiratórias,
                além de melhorar o desempenho físico e a autoestima.
              </p>
              <p>
                No Brasil, o 
                <a href="https://www.gov.br/inca/pt-br" target="_blank">
                  Instituto Nacional de Câncer (INCA)
                </a> 
                coordena o Programa Nacional de Controle do Tabagismo (PNCT),
                que oferece diversas opções de tratamento para os fumantes, como
                a terapia de reposição de nicotina (TRN), medicamentos
                prescritos e terapia comportamental. A TRN, por exemplo,
                consiste no uso de produtos que liberam nicotina de forma
                gradual, reduzindo os sintomas da abstinência.
              </p>
              <p>
                A 
                <a href="https://www.paho.org/pt" target="_blank">
                  Organização Mundial da Saúde (OMS)
                </a> 
                destaca a importância da cessação do tabagismo para alcançar as
                metas globais de saúde. No entanto, muitos fumantes enfrentam
                dificuldades para abandonar o vício. Neste contexto, o PNCT
                desempenha um papel fundamental ao oferecer suporte e orientação
                aos fumantes que desejam mudar de vida.
              </p>
            </section>
            <section>
              <h3>Métodos comuns para cessação do tabagismo</h3>
              <p>
                Alguns dos métodos mais comuns utilizados na cessação do
                tabagismo incluem:
              </p>
              <ul>
                <li>
                  Terapia de Reposição de Nicotina (TRN): envolve o uso de
                  produtos que fornecem doses controladas de nicotina para
                  reduzir gradualmente a dependência. Os formatos incluem
                  adesivos, chicletes, pastilhas, inaladores e sprays nasais. A
                  TRN é eficaz em aliviar os sintomas de abstinência e pode
                  dobrar as taxas de sucesso na cessação do tabagismo comparado
                  ao uso de placebos. No entanto, pode haver efeitos colaterais
                  como irritação na pele, problemas digestivos e insônia.
                </li>
                <li>
                  Medicamentos Prescritos: medicamentos como bupropiona e
                  vareniclina são frequentemente prescritos para ajudar na
                  cessação do tabagismo. A bupropiona atua como um
                  antidepressivo que também ajuda a reduzir os desejos e
                  sintomas de abstinência. A vareniclina bloqueia os receptores
                  de nicotina no cérebro, reduzindo o prazer derivado do tabaco
                  e os sintomas de abstinência. Esses medicamentos podem ser
                  altamente eficazes, mas apresentam possíveis efeitos
                  colaterais, como alterações de humor, insônia e, em alguns
                  casos, pensamentos suicidas.
                </li>
                <li>
                  Terapia Comportamental: a terapia comportamental envolve
                  sessões individuais ou em grupo com um profissional de saúde
                  mental, focando em técnicas de modificação do comportamento,
                  gestão de estresse e desenvolvimento de estratégias para lidar
                  com os gatilhos de fumar. Este método pode ser muito eficaz,
                  especialmente quando combinado com outros métodos de cessação,
                  como a TRN ou medicamentos. A principal vantagem da terapia
                  comportamental é a abordagem personalizada e o suporte
                  contínuo, embora o acesso e o custo possam ser barreiras para
                  alguns indivíduos.
                </li>
                <li>
                  Técnicas Alternativas: como hipnose, acupuntura e terapia a
                  laser, têm ganhado popularidade como métodos de cessação do
                  tabagismo. A eficácia dessas abordagens varia e muitas vezes
                  carece de suporte científico robusto. Alguns fumantes relatam
                  sucesso com essas técnicas, enquanto outros não observam
                  diferença significativa. Elas podem ser opções valiosas para
                  aqueles que não respondem bem aos métodos tradicionais ou que
                  buscam abordagens complementares.
                </li>
              </ul>
            </section>
            <section>
              <h3>Dicas para ajudar você a parar de fumar com mais sucesso</h3>
              <p>
                Deixar o cigarro é uma mudança que traz benefícios quase
                imediatos, mas pode ser difícil. O tabagismo é classificado como
                uma doença crônica, o que significa que recaídas são comuns e
                muitos fumantes precisam de várias tentativas para conseguir
                parar de fumar.
              </p>
              <ul>
                <li>
                  Tenha determinação: decidir parar de fumar é o primeiro passo.
                  A motivação deve vir de dentro, e é importante focar nos
                  benefícios da abstinência em vez de sentir-se privado.
                </li>
                <li>
                  Marque um dia para parar: escolha uma data próxima para parar
                  de fumar. Essa data pode ser significativa, mas não se
                  concentre apenas nisso. Consultar um profissional de saúde
                  pode ser útil, mas algumas pessoas conseguem parar por conta
                  própria.
                </li>
                <li>
                  Corte gatilhos do fumo: identifique e evite os gatilhos que
                  fazem você querer fumar, como álcool e café. Substitua esses
                  momentos por atividades que não estejam associadas ao
                  tabagismo, como beber água ou comer frutas.
                </li>
                <li>
                  Escolha um método: decida se vai parar de fumar abruptamente
                  ou gradualmente. Parar de uma vez pode causar sintomas de
                  abstinência, enquanto reduzir gradualmente pode ajudar a
                  evitar recaídas.
                </li>
                <li>
                  Encontre substitutos saudáveis: procure alternativas saudáveis
                  para combater a vontade de fumar, como beber água, comer
                  cenouras cruas ou picolés de frutas. A atividade física também
                  ajuda a reduzir a fissura.
                </li>
                <li>
                  Livre-se das lembranças do cigarro: remova itens associados ao
                  cigarro, como cinzeiros. Alterar a rotina e a disposição da
                  casa pode ajudar a evitar os gatilhos.
                </li>
                <li>
                  Encontre apoio de amigos e familiares: o apoio de amigos e
                  familiares é crucial, mas deve ser dado de forma positiva e
                  não como pressão. Ofereça suporte com ações práticas, como
                  levar água ou propor atividades.
                </li>
                <li>
                  Escolha a melhor alimentação: mantenha uma dieta saudável e
                  evite alimentos que possam aumentar a vontade de fumar. Opte
                  por alimentos e bebidas que ajudem a manter a mente ocupada e
                  o corpo saudável.
                </li>
                <li>
                  Procure apoio médico: busque orientação de profissionais de
                  saúde, que oferecem tratamento gratuito pelo Sistema Único de
                  Saúde(SUS). O tratamento pode incluir medicamentos e
                  acompanhamento especializado.
                </li>
                <li>
                  Troque experiências em um grupo de apoio: participar de grupos
                  de apoio pode ser muito útil. Compartilhar experiências e
                  receber conselhos de pessoas com o mesmo objetivo pode
                  aumentar suas chances de sucesso.
                </li>
              </ul>
              <p>
                Lembre-se, você não está sozinho nessa jornada! Junte-se ao
                movimento Cigarro Zero e conquiste a liberdade do tabaco.
              </p>
            </section>
    `,
  },
  "o-papel-das-midias-sociais-na-promocao-do-tabagismo-entre-os-jovens": {
    title:
      "O Papel das Mídias Sociais na Promoção do Tabagismo entre os Jovens",
    subtitle:
      "Cigarro e redes sociais: uma combinação explosiva para sua saúde.",
    heroImage: OPapelDasMidiasSociaisNaPromocaoDoTabagismoEntreOsJovensImg,
    writer: {
      photo: RobertaEscritoraImg,
      name: "Roberta Braz",
      articleDate: "12 de agosto de 2024",
    },
    article: `
            <section>
              <p>
                A cada dia, milhares de jovens brasileiros iniciam o vício do
                cigarro, muitos deles influenciados pelas redes sociais. Dados
                da última Pesquisa Nacional de Saúde do Escolar (PeNSE), revelou
                que, em 2019, 16,8% dos escolares de 13 a 17 anos já haviam
                experimentado o cigarro eletrônico, sendo 13,6% nos de 13 a 15
                anos e 22,7% nos de 16 e 17 anos. Quanto ao sexo, a
                experimentação é maior entre os homens (18,1%) do que entre as
                mulheres (14,6%). As crianças e adolescentes que usam cigarros
                eletrônicos têm pelo menos duas vezes mais probabilidade de
                fumar cigarros mais tarde na vida.
              </p>
              <p>
                A entrada no vício durante a adolescência está associada a
                diversas razões, incluindo, a imitação do comportamento do
                grupo, amigos e pais tabagistas. Um outro fator que contribui
                para esse quadro é o fácil acesso, apontado como um dos
                principais motivos de iniciação e indução ao consumo.
              </p>
              <p>
                Além disso, influenciadores e celebridades frequentemente postam
                fotos e vídeos onde aparecem fumando ou usando produtos de
                tabaco, muitas vezes sem qualquer menção direta ao produto. Essa
                exposição indireta normaliza o comportamento e o torna atraente
                para os jovens.
              </p>
            </section>
            <section>
              <h3>Impactos da promoção do tabagismo nas mídias sociais</h3>
              <ul>
                <li>
                  Normalização do comportamento: um dos maiores impactos da
                  promoção do tabagismo nas mídias sociais é a normalização
                  desse comportamento. Quando os jovens veem seus amigos,
                  influenciadores e celebridades fumando, eles podem começar a
                  ver o tabagismo como uma atividade socialmente aceitável e
                  desejável. Isso pode levar à iniciação precoce e ao
                  desenvolvimento de hábitos de fumo que podem durar a vida
                  inteira.
                </li>
                <li>
                  Redução da percepção dos riscos: as mídias sociais têm o
                  potencial de reduzir a percepção dos riscos associados ao
                  tabagismo. A representação glamorosa e muitas vezes
                  inconsequente do fumo pode fazer com que os jovens subestimem
                  os perigos reais, como doenças cardíacas, câncer e
                  dependência. A falta de informação precisa e a glamorização do
                  comportamento contribuem para uma visão distorcida do
                  tabagismo.
                </li>
              </ul>
            </section>
            <section>
              <h3>Como podemos combater a promoção do tabagismo?</h3>
              <ul>
                <li>
                  Educação e conscientização: uma das estratégias mais eficazes
                  para combater a promoção do tabagismo nas mídias sociais é
                  aumentar a educação e a conscientização entre os jovens sobre
                  os riscos do tabagismo. Campanhas educacionais que utilizam as
                  mesmas plataformas de mídias sociais podem alcançar um grande
                  público e fornecer informações precisas e impactantes.
                </li>
                <li>
                  Políticas e regulamentações: as plataformas de mídias sociais
                  devem ser incentivadas a implementar e reforçar políticas que
                  proíbam a promoção de produtos de tabaco. Além disso, a
                  regulamentação governamental pode desempenhar um papel
                  essencial na limitação da publicidade de tabaco nas mídias
                  sociais, incluindo a promoção indireta por influenciadores.
                </li>
                <li>
                  Envolvimento da comunidade: o envolvimento da comunidade,
                  incluindo pais, escolas e organizações de saúde, é essencial
                  para criar um ambiente que desencoraje o tabagismo. Programas
                  de prevenção do tabagismo que envolvem a participação ativa da
                  comunidade podem ajudar a construir uma rede de apoio para
                  jovens, reduzindo a pressão para experimentar o tabaco.
                </li>
              </ul>
              <p>Junte-se a nós nesse movimento Cigarro Zero!</p>
            </section>
    `,
  },
  "cigarro-eletronico-mitos-e-verdades": {
    title: "Cigarro Eletrônico: Mitos e Verdades",
    subtitle: "Um novo visual, os mesmos perigos!",
    heroImage: CigarroEletronicoMitosEVerdadesImg,
    writer: {
      photo: RobertaEscritoraImg,
      name: "Roberta Braz",
      articleDate: "12 de agosto de 2024",
    },
    article: `
            <section>
              <p>
                Os cigarros eletrônicos, ou vapes, têm ganhado crescente
                popularidade, sendo promovidos como uma alternativa supostamente
                mais segura ao cigarro tradicional. Desde sua introdução em
                2003, esses dispositivos passaram por uma evolução
                significativa, envolvendo várias gerações de produtos.
                Inicialmente, surgiram os modelos descartáveis, projetados para
                uso único. Em seguida, apareceram os dispositivos recarregáveis,
                que utilizam refis líquidos em sistemas abertos ou fechados.
              </p>
              <p>
                Além disso, a indústria introduziu dispositivos de tabaco
                aquecido, que utilizam um refil sólido, geralmente contendo
                tabaco. Os cigarros eletrônicos também ampliaram suas
                possibilidades com o uso de nicotina em formas diferenciadas,
                como sais de nicotina e nicotina sintética, além de permitir o
                consumo de outras plantas e substâncias alternativas ao tabaco.
              </p>
              <p>
                Apesar de sua crescente popularidade, é essencial esclarecer que
                os cigarros eletrônicos não são uma alternativa inofensiva.
                Segundo o
                <a
                  href="https://www.gov.br/inca/pt-br/assuntos/gestor-e-profissional-de-saude/observatorio-da-politica-nacional-de-controle-do-tabaco/politica-nacional/dispositivos-eletronicos-para-fumar-def-1"
                  target="_blank"
                >
                  Instituto Nacional de Câncer (INCA)
                </a>
                , os Dispositivos Eletrônicos para Fumar (DEFs) não são seguros,
                pois contêm substâncias tóxicas, além da nicotina. Esses
                dispositivos podem causar diversas doenças graves, incluindo
                enfisema pulmonar, doenças cardiovasculares, dermatite e câncer.
                Estudos também mostram que os níveis de toxicidade dos cigarros
                eletrônicos podem ser tão prejudiciais quanto os dos cigarros
                tradicionais, uma vez que combinam substâncias tóxicas que
                mascaram os efeitos danosos.
              </p>
            </section>
            <section>
              <h3>O cigarro eletrônico é inofensivo para a saúde?</h3>
              <p>
                Embora não contenha tabaco, o vape libera substâncias químicas
                que podem causar danos à saúde, como metais pesados, partículas
                finas e compostos orgânicos voláteis. Além disso, a nicotina
                presente no líquido para vape causa dependência e pode levar a
                problemas cardíacos e respiratórios.
              </p>
            </section>
            <section>
              <h3>O cigarro eletrônico ajuda a parar de fumar?</h3>
              <p>
                Alguns estudos sugerem que o cigarro eletrônico pode auxiliar na
                cessação do tabagismo tradicional, ao fornecer a nicotina de
                forma diferente. No entanto, muitos ex-fumantes acabam usando
                ambos os produtos simultaneamente ou trocam um vício pelo outro.
              </p>
            </section>
            <section>
              <h3>O vapor do cigarro eletrônico não faz mal aos outros?</h3>
              <p>
                Assim como a fumaça do cigarro tradicional, o vapor do cigarro
                eletrônico contém partículas que podem ser prejudiciais à saúde
                das pessoas ao redor, especialmente crianças e pessoas com
                problemas respiratórios.
              </p>
            </section>
            <section>
              <h3>O cigarro eletrônico é regulamentado e seguro?</h3>
              <p>
                A regulamentação do cigarro eletrônico varia de país para país.
                No Brasil, a Agência Nacional de Vigilância Sanitária (Anvisa),
                publicou a
                <a
                  href="https://www.in.gov.br/web/dou/-/resolucao-da-diretoria-colegiada-rdc-n-855-de-23-de-abril-de-2024-555721206"
                  target="_blank"
                >
                  Resolução da Diretoria Colegiada (RDC) nº 855, de 23 de abril
                  de 2024
                </a>
                , proibido a fabricação, a importação, a comercialização, a
                distribuição, o armazenamento, o transporte e a propaganda de
                dispositivos eletrônicos.
              </p>
            </section>
            <section>
              <h3>O cigarro eletrônico não causa câncer?</h3>
              <p>
                Embora existam menos estudos sobre o cigarro eletrônico em
                comparação ao cigarro tradicional, algumas pesquisas indicam que
                o vape pode aumentar o risco de desenvolver certos tipos de
                câncer, especialmente os relacionados ao sistema respiratório.
              </p>
              <p>
                A ideia de que os cigarros eletrônicos são uma solução mágica
                para deixar de fumar é enganosa. Qualquer produto que contenha
                nicotina é viciante e pode trazer sérios riscos à saúde.
                Portanto, a melhor forma de proteger sua saúde é evitar tanto o
                cigarro tradicional quanto o eletrônico, buscando alternativas
                saudáveis, como terapia e acompanhamento médico, para lidar com
                o vício e adotar um estilo de vida mais saudável.
              </p>
              <p>Junte-se a nós nesse movimento Cigarro Zero!</p>
            </section>
    `,
  },
};

export const contentsSlugs = Object.keys(contents);

export default contents;
