...
//           if (map) {
//             map.setCenter(userLatLng);
//             fetchPostosDeSaude(); // Carrega os postos próximos à localização do usuário
//           }
//         },
//         () => {
//           if (map) {
...
const libraries: ("places" | "geometry")[] = ["places", "geometry"]; // Incluímos a biblioteca de geometria
  ...
  const fetchPostosDeSaude = useCallback(() => {
    if (window.google && window.google.maps && userLocation) {
      ...
      const requests = keywords.map((keyword) => ({
        ...
        radius: 5000, // Define um raio de 5 km
        keyword: keyword,
      }));
        ...
        placesService.nearbySearch(request, (results, status) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
            const postos = results.map((place) => {
              ...
              // Verifica se o posto está dentro de 5 km da localização do usuário
              ...
              return null; // Ignora marcadores fora do raio
            }).filter(marker => marker !== null); // Remove marcadores nulos
